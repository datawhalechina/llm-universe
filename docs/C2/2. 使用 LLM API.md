...
#### 配置 Base url（若使用的为官网获取的API，可忽略该步骤）

如果你的api非官网获取，则可能为**转发api**，请根据购买时的收货信息，找到`转发/接口 地址`或类似字样，复制后面网址以此形式`OPENAI_BASE_URL="https://..."` 保存到刚才新建的 `.env` 文件中。
