...
如果你需要在 LangChain 中使用 LLM，可以参照[LLM 接入 LangChain](https://github.com/datawhalechina/llm-universe/blob/main/notebook/C4%20%E6%9E%84%E5%BB%BA%20RAG%20%E5%BA%94%E7%94%A8/C4.ipynb)中的调用方式。
...
1. 首先，让我们调用 OpenAI 的 API ，封装一个对话函数，使用 gpt-3.5-turbo 这个模型。
    ...
    注：如果你使用的是其他模型 API，请参考[第二节内容](#22-使用-llm-api)修改下文的 `get_completion` 函数。
