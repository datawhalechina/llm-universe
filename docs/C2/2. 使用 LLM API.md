...
注意修改`.env`文件后需要重启(Restart) Jupyter以确保环境变量被刷新。
...
`github`上也有很多项目提供免费的国内可使用的`API KEY`，虽然有请求次数限制，但如果你仅仅想尝试使用`GPT`，可以参照下面的流程。

前往[GPT-API-free](https://github.com/chatanywhere/GPT_API_free)，申请领取内测免费`API Key`，并保存记录`API Key`

在代码中填写api_key，注意修改base_url为`https://api.chatanywhere.tech/v1`，示例代码如下：
...
请注意免费`API Key`仅可用于个人非商业用途，教育，非营利性科研工作中。免费API Key严禁商用，严禁大规模训练商用模型！
