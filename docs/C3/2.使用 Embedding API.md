...
## 三、使用Ernie Bot API
相较于文心千帆API，`Ernie Bot API` 的调用方式更为简单，但是 Ernie Bot API 目前支持的 Embedding 模型只有自家的 `"ernie-text-embedding"`。
...
text = '要生成 embedding 的输入文本，字符串形式。'
response= ernie_embedding(text=text)
```

和文心千帆API中的 Embedding-V1 一样。除了有单独的id外，还有时间戳记录embedding的时间。
...
```
    ...
    本次embedding id为：as-zyj9pqhwr1
    本次embedding产生时间戳为：1713353183
